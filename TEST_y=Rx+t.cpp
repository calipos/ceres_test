// g++ TEST_optimize_rotation.cpp
// -I/home/tx2/repo/ceres-solver/build/install/usr/local/include
// -I/usr/local/include/eigen3 -I/usr/include
// -L/home/tx2/repo/ceres-solver/build/install/usr/local/lib -lceres -lglog
// -lpthread -lopenblas
...
#endif
#endif
#define RandUnit (rand() % 1000 / 500. - 1)
#define UseAutoDiff 1
struct RotationError {
  RotationError(double before_x_,
                double before_y_,
                double before_z_,
                double after_x_,
                double after_y_,
  ...
  bool operator()(const T* const rVec, T* residuals) const {
    T p[3];
    T beforeXYZ[3] = {static_cast<T>(before_x),
                      static_cast<T>(before_y),
                      static_cast<T>(before_z)};
    ceres::AngleAxisRotatePoint(rVec, beforeXYZ, p);
    residuals[0] = after_x - p[0] - rVec[3];
    residuals[1] = after_y - p[1] - rVec[4];
    residuals[2] = after_z - p[2] - rVec[5];
    return true;
  }
  double before_x, before_y, before_z;
  double after_x, after_y, after_z;
};
struct RotationError2 : public ceres::SizedCostFunction<3, 6> {
  RotationError2(
  ...
  virtual bool Evaluate(double const* const* parameters,
    ...
    y = R * x;
    residuals[0] = y.ptr<double>(0)[0] + parameters[0][3] - y1_;
    residuals[1] = y.ptr<double>(1)[0] + parameters[0][4] - y2_;
    residuals[2] = y.ptr<double>(2)[0] + parameters[0][5] - y3_;
    if (jacobians != NULL && jacobians[0] != NULL) {
      ...
      jacobians[0][4] = 0;
      jacobians[0][5] = 0;
      jacobians[0][6] = residualMat.ptr<double>(0)[0];
      jacobians[0][7] = residualMat.ptr<double>(1)[1];
      jacobians[0][8] = residualMat.ptr<double>(1)[2];
      jacobians[0][9] = 0;
      jacobians[0][10] = 1;
      jacobians[0][11] = 0;
      jacobians[0][12] = residualMat.ptr<double>(2)[0];
      jacobians[0][13] = residualMat.ptr<double>(2)[1];
      jacobians[0][14] = residualMat.ptr<double>(2)[2];
      jacobians[0][15] = 0;
      jacobians[0][16] = 0;
      jacobians[0][17] = 1;
    }
...
cv::Mat rotationVectorToMatrix(const double& a,
                               const double& b,
                               const double& c) {
  ...
  K.ptr<Dtype>(2)[2] = 0;
  cv::Mat R = cv::Mat::eye(3, 3, sizeof(Dtype) / 4 + 4) +
              (1 - cos(theta)) * K * K + sin(theta) * K;
  return R;
}
template <class Dtype>
...
cv::Point3_<Dtype> matrixToZyx(const cv::Mat& m) {
  ...
  zyx.y = atan2(-m.ptr<Dtype>(2)[0],
                sqrt(m.ptr<Dtype>(2)[2] * m.ptr<Dtype>(2)[2] +
                     m.ptr<Dtype>(2)[1] * m.ptr<Dtype>(2)[1]));
...
int  TESTrotation() {
  ...
  double gtt[3] = {RandUnit * 10, RandUnit * 10, RandUnit * 10};
  for (int i = 0; i < sampleNum; i++) {
    ...
    after[3 * i] = p[0] + gtt[0];    // 0.01 * RandUnit;
    after[3 * i + 1] = p[1] + gtt[1];  // 0.01 * RandUnit;
    after[3 * i + 2] = p[2] + gtt[2];  // 0.01 * RandUnit;
  }
  // double rVec[3]={0.1,1,-0.5};
  double* rVec = new double[6];
  rVec[0] = 0.1;
  rVec[1] = 0.2;
  rVec[2] = 3;
  rVec[3] = 0;
  ...
  for (int i = 0; i < sampleNum; ++i) {
    ...
    ceres::CostFunction* cost_function =
        new ceres::AutoDiffCostFunction<RotationError, 3, 6>(
            new RotationError(before[3 * i],
                              before[3 * i + 1],
                              before[3 * i + 2],
                              after[3 * i],
                              after[3 * i + 1],
    ...
    problem.AddResidualBlock(cost_function,
                             ...
                             rVec);
#else
    ceres::CostFunction* cost_function = new RotationError2(before[3 * i],
                                                            before[3 * i + 1],
                                                            before[3 * i + 2],
                                                            after[3 * i],
                                                            after[3 * i + 1],
  ...
  LOG(INFO) << t;
  LOG(INFO) << "gt=" << axisDir;
  LOG(INFO) << "gtt=" << gtt[0] << " " << gtt[1] << " " << gtt[2] ;
  return 0;
}
